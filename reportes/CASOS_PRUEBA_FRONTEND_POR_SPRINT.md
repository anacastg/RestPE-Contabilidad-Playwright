...
>
> **Fecha:** 17/06/2026 · **Revisión:** 22/06/2026 (v3.1 — alineado con listado CSV)
>
> **HU Referenciadas:**
>
...
## Índice de alineación con listado CSV

> Referencia cruzada: ítem `#` del CSV → sección del documento. Los ítems `#62` y sub-ítems `#14a/#14b` se cubren dentro de `#61` y `#14` respectivamente.

| # | Sprint | Menú (CSV) | Sección MD |
|:-:|--------|------------|------------|
| 1 | S1 | Maestro Proveedores | 1.1 |
| 2 | S1 | Orden de Compra > Edición General | 1.2 |
| 3 | S1 | Aprobación de OC y OS | 1.3 |
| 4 | S1 | Orden de Servicios > Generación | 1.4 |
| 5 | S1 | Orden de Servicios > Aprobación Acta | 1.4 |
| 6 | S1 | Cuentas por Pagar > Registro CxP | 1.5 |
| 7 | S1 | NC/ND CxP | 1.6 |
| 8 | S1 | Documentos x Pagar Directo | 1.7 |
| 9 | S1 | Reporte gestión compras al detalle | 1.8 |
| 10 | S1 | Registro de Ventas SUNAT | 1.9 |
| 11 | S1 | Reporte Tributario | 1.9 |
| 15–17, 33 | S1 | Maestros Finanzas / TC | 1.10 |
| 12–14 | S2 | Tablas Finanzas | 2.1 |
| 18–19 | S2 | Adelantos > OG | 2.2 |
| 20–22 | S2 | Rendición / cierre adelantos | 2.3 |
| 23–26 | S2 | Tesorería | 2.4 |
| 27 | S2 | Transferencias | 2.5 |
| 28–32, 34 | S2 | Tablas Contables | 2.6 |
| 35–37 | S2 | Reportes / PLE | 2.7 |
| 38–52 | S3 | Activos Fijos | 3.1–3.4 |
| 53–55 | S3 | RRHH Configuración | 3.5 |
| 56, 58 | S3 | RRHH Datos del Personal | 3.6 |
| 57, 59–73 | S4 | RRHH Operaciones / Planilla | 4.1–4.13 |

**Flujos scripteados (3 por sprint):** S1 → §1.11 · S2 → §2.8 · S3 → §3.7 · S4 → §4.14
...
>
> Operaciones: Crear OC, grabar líneas, numeración y consulta. Generar PDF
...
## 1.3 Aprobación de OC y OS (#3)

> **HU asociada:** `HU-COM-004` — Aprobación de Orden de Compra
>
> Ruta: `Compras > Aprobación de OC y OS`
>
> Operaciones: Aprobar/rechazar OC pendiente
...
## 1.4 Orden de Servicios — Generación (#4) y Aprobación de Acta (#5)

> Rutas:
> - `#4` — `Compras > Orden de Servicios > Generación`
> - `#5` — `Compras > Orden de Servicios > Aprobación de Acta de conformidad`
...
> Ruta: `Compras > Gestión de compras > Gestión de compras al detalle`
>
> SP: `compras.sp_generar_reporte_compras`
...
> Rutas:
> - `#10` — `Ventas > Reporte de Ventas > Registro de Ventas > Formato de SUNAT` (SP: `ventas.sp_generar_registro_ventas`)
> - `#11` — `Ventas > Reporte Tributario > Generación por periodo (IGV, Retenciones)`
...
> Rutas:
> - `#15` — `Finanzas > Bancos > Cuentas de Bancos` · `/configuracion/localizacion/cuenta-bancaria`
> - `#16` — `Finanzas > Tablas de SUNAT > Medios de PAGO`
> - `#17` — `Finanzas > Tablas de SUNAT > Formas de Pago`
> - `#33` — `Contabilidad > Tipo de cambio` (no aplica EC)
...
### CP-S1-047: CRUD Tipo de Cambio (#33)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔵 Funcional |
| **Descripción** | Registrar TC y verificar lectura vía `fn_obtener_tipo_cambio` en pantallas que lo usen |
| **Precondiciones** | — |
...
### CP-S1-E2E-01: Compras E2E — Proveedor (#1) → OC (#2) → Aprobación (#3) → CxP (#6) → Reporte (#9)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S1: proveedor → OC → aprobación → factura CxP → reporte de compras |
| **Precondiciones** | Datos maestros básicos cargados |
| **Pasos** | 1. Crear proveedor (#1). 2. Crear OC con líneas (#2). 3. Aprobar OC (#3). 4. Registrar factura CxP vinculada (#6). 5. Generar reporte gestión de compras (#9) |
...
### CP-S1-E2E-02: Ventas Reportes — Registro (#10) + Tributario (#11)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S1: registro de ventas SUNAT y reporte tributario del mismo período |
| **Precondiciones** | Ventas registradas en el período |
| **Pasos** | 1. Generar Registro de Ventas SUNAT (#10). 2. Generar Reporte Tributario IGV/Retenciones (#11). 3. Comparar totales de IGV |
| **Esperado** | Totales de IGV coinciden entre ambos reportes para el mismo período |

### CP-S1-E2E-03: Finanzas Base — Cuenta (#15) + Medio/Forma Pago (#16/#17) + TC (#33)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S1: maestros financieros base integrados en transacciones |
| **Precondiciones** | — |
| **Pasos** | 1. Crear cuenta bancaria (#15). 2. Crear medio de pago (#16). 3. Crear forma de pago (#17). 4. Registrar TC (#33). 5. Verificar disponibilidad en combos operativos |
...
### CP-S2-021: Formato 5.2 — Libro Diario Simplificado (SUNAT) (#35)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔵 Funcional |
| **Descripción** | Generar libro diario simplificado |
| **Precondiciones** | Asientos contables registrados |
| **Pasos** | 1. Contabilidad > Formatos SUNAT > Formato 5.2 Libro Diario Simplificado. 2. Seleccionar período. 3. Ejecutar (`sp_generar_libro_diario_simplificado`). 4. Exportar |
| **Esperado** | Tabla con formato SUNAT. Columnas: cuenta, descripción, debe, haber. Totales deben coincidir |

### CP-S2-022: Formatos SUNAT básicos (#36)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔵 Funcional |
| **Descripción** | Generar reportes SUNAT básicos del catálogo de formatos |
| **Precondiciones** | Datos contables del período |
| **Pasos** | 1. Contabilidad > Formatos SUNAT (formatos básicos). 2. Seleccionar período y formato. 3. Ejecutar (`sp_generar_reporte_sunat`). 4. Exportar |
| **Esperado** | Reporte generado sin errores. Columnas y totales coherentes con el formato SUNAT seleccionado |

### CP-S2-023: Procesar PLE — Libros Electrónicos SUNAT (#37)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔵 Funcional |
| **Descripción** | Ejecutar proceso de generación de libros electrónicos |
| **Precondiciones** | Datos contables del período |
| **Pasos** | 1. Contabilidad > Libros Electrónicos (SUNAT). 2. Seleccionar período y libros a incluir. 3. Ejecutar (`sp_generar_libros_electronicos`). 4. Ver resultado |
...
### CP-S2-E2E-01: Adelantos → Tesorería — OG (#18) → Aprobación (#19) → Liquidación (#20/#21) → Cierre (#22) → Cartera (#23)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S2: cadena completa de adelantos hasta cartera de pagos |
| **Precondiciones** | Colaborador, conceptos, cuentas bancarias configurados |
| **Pasos** | 1. Crear OG (#18). 2. Aprobar OG (#19). 3. Rendir gastos (#20). 4. Aprobar rendición (#21). 5. Cerrar liquidación (#22). 6. Programar pago en cartera (#23) |
...
### CP-S2-E2E-02: Cartera CxC — Cobro + Aplicación (+ anulación opcional #26)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S2: cobro #24 → aplicación #25; opcionalmente anulación en cartera de pagos #26 |
| **Precondiciones** | Documento de venta pendiente |
| **Pasos** | 1. Registrar cobro en cartera de cobros (#24). 2. Aplicar pago a documentos abiertos (#25). 3. *(Opcional)* Anular pago emitido en cartera de pagos (#26) |
| **Esperado** | Tras aplicar: documento como cobrado. Si se ejecuta #26: el pago se reversa y el documento vuelve a pendiente |

### CP-S2-E2E-03: Contabilidad — Plan (#28) + Libro Diario (#35) + PLE (#37)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S2: plan contable, libro diario y PLE con SP desplegados |
| **Precondiciones** | Asientos contables registrados |
| **Pasos** | 1. Verificar/ajustar plan contable (#28). 2. Generar libro diario Formato 5.2 (#35). 3. Procesar PLE (#37). 4. Comparar consistencia |
...
> Rutas:
> - `#56` — `RR.HH > Datos del Personal > Datos Generales` · `/rrhh/maestro-personal/datos-contacto`
> - `#58` — `RR.HH > Categorías/Cargos` · `/rrhh/maestro-personal/definicion-cargos`
...
### CP-S3-E2E-01: Activos Fijos E2E — Alta (#38) → Depreciación (#49) → Asientos (#50)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S3: alta activo → cálculo depreciación → generación asientos |
| **Precondiciones** | Clasificación con cuentas contables, parámetros de depreciación |
| **Pasos** | 1. Alta de activo (#38). 2. Calcular depreciación del período (#49, `sp_calcular_depreciacion`). 3. Generar asientos de depreciación (#50). 4. Verificar asiento en contabilidad si integrado |
...
### CP-S3-E2E-02: Revaluación (#45) + Asientos (#51)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S3: revaluación y asientos de revaluación |
| **Precondiciones** | Activo con antigüedad |
| **Pasos** | 1. Ejecutar revaluación (#45, `sp_revaluacion_activo`). 2. Generar asientos de revaluación (#51) |
| **Esperado** | Activo refleja el nuevo valor. Asientos de revaluación debitan/créditan cuentas de activo y patrimonio |

### CP-S3-E2E-03: RRHH Maestros — Período (#53) + Trabajador (#56) + Cargo (#58)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S3: maestros básicos de RRHH |
| **Precondiciones** | — |
| **Pasos** | 1. Definir período de nómina (#53). 2. Crear ficha de trabajador (#56). 3. Crear cargo con banda salarial (#58) y asignarlo al trabajador |
...
> Rutas:
> - `#57` — `RR.HH > Tipo de contrato` · `/rrhh/parametros/tipo-contrato`
> - `#59` — `RR.HH > Maestros > Administradoras de A.F.P.`
> - `#60` — `RR.HH > Maestros > Calendario feriados (+ calendarios laborales)`
...
## 4.2 Asistencias y Jornadas (#61, #62 integrado)

> Ruta: `RR.HH > Asistencias y horas extra` · `/rrhh/asistencias-jornadas/asistencias-HE`
>
> Nota CSV: al guardar asistencia (#61) se calculan HE (#62 integrado en la misma pantalla)
...
> Alineados con los 3 flujos scripteados del listado CSV (Sprint 4).

### CP-S4-E2E-01: Asistencia (#61) con HE → Planilla (#69)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S4: registrar asistencia con HE y calcular planilla del mismo período |
| **Precondiciones** | Trabajador activo, período de nómina abierto, conceptos fijos configurados |
| **Pasos** | 1. Registrar asistencia con horas extra (#61; HE #62 al guardar). 2. Ejecutar cálculo de planilla (#69, `sp_calcular_planilla`) para el mismo período |
| **Esperado** | Las HE registradas impactan el cálculo de planilla. Neto y conceptos variables coherentes con la asistencia |

### CP-S4-E2E-02: Liquidación — Finiquito (#70)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S4: finiquito en régimen general |
| **Precondiciones** | Trabajador activo con antigüedad |
| **Pasos** | 1. Liquidaciones de Trabajador (#70, `sp_liquidar_beneficios`). 2. Validar truncas vía FN del SP |
| **Esperado** | Beneficios truncos proporcionales calculados. Documento de liquidación generado |

### CP-S4-E2E-03: Cierre de nómina — Gratificación/CTS (#71) → Netos (#72) → Boleta (#73)
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **Descripción** | Flujo scripteado S4: gratificación y CTS, consulta de netos y emisión de boletas |
| **Precondiciones** | Planilla calculada, trabajadores activos |
| **Pasos** | 1. Calcular gratificación y CTS (#71, `sp_calcular_gratificacion`, `sp_calcular_cts`). 2. Consultar saldos de cuenta corriente (#72, `sp_generar_pago_remuneraciones`). 3. Emitir boletas (#73, `sp_generar_boleta_pago`) |
| **Esperado** | Netos y adelantos consistentes. Boletas PDF con todos los conceptos del período |

---

## 4.15 Casos E2E ampliados por HU (complementarios)

> Flujos adicionales de integración por Historia de Usuario. No forman parte de los 3 casos scripteados del CSV, pero amplían la cobertura de regresión.

### CP-S4-E2E-HU-01: Asistencia → Variables → Propinas → Planilla → Boletas
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **HU Ref** | HU-RRHH-PN-CP-001 + HU-RRHH-PN-CP-002 + HU-RRHH-PN-CP-003 |
| **Descripción** | Flujo ampliado: asistencia con HE + carga variables → propinas → planilla → boletas |
...
| **Esperado** | La boleta incluye: sueldo base, HE, bono, propinas (ingresos) y AFP, IRPF, préstamo (descuentos). Neto correcto. Trazabilidad completa |

### CP-S4-E2E-HU-02: Recargo al Consumo + Aportes → Planilla → Boletas
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **HU Ref** | HU-RRH-NOM-CP-007 + HU-RRHH-NOM-AR-001 |
| **Descripción** | Flujo ampliado: recargo al consumo + aportes patronales en planilla |
...
| **Esperado** | Recargo al consumo como ingreso separado. Aportes desglosados (trabajador / empleador). Neto correcto |

### CP-S4-E2E-HU-03: Adelantos + Préstamos → Planilla → Saldos
| Campo | Detalle |
|-------|---------|
| **Tipo** | 🔗 Integración |
| **HU Ref** | HU-RRHH-NOM-CC-001 |
| **Descripción** | Flujo ampliado: adelanto/préstamo → descuento en planilla → consulta saldos |
| **Precondiciones** | Trabajador activo |
| **Pasos** | 1. Registrar adelanto de sueldo. 2. Registrar préstamo con cuotas. 3. Calcular planilla. 4. Consultar saldos de cuenta corriente |
| **Esperado** | Planilla descuenta adelanto y cuota de préstamo. Saldo pendiente reflejado en cuenta corriente |
...
| **S2** | Finanzas + Contabilidad | 18 | 2 | 1 | 0 | 5 | **26** | — |
| **S3** | Activos Fijos + RRHH inicio | 23 | 3 | 4 | 0 | 3 | **33** | 15 HU |
| **S4** | RR.HH completo | 32 | 8 | 8 | 1 | 6 | **58** | 7 HU |
| | **Totales** | **103** | **19** | **22** | **4** | **17** | **168** | **27 HU** |
...
*Documento generado el 17/06/2026 — v3.1 — 168 casos, 27 HU trazadas — alineado con listado CSV Sprint 1–4 — QA Senior - Frontend Testing*
